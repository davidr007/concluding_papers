...
%DELTA_t = DELTA_t.*U;
DELTA_t_sq= (FDOM/vs).^2.*((XVEC_P - XVEC_Q).^2 + (YVEC_P - YVEC_Q).^2);
deriv_deltat_xp = zeros(n); 
deriv_deltat_yp = zeros(n); 

deriv_deltat_xp(U==1) = FDOM(U==1).^2./vs^2.*2.*(XVEC_P(U==1)-XVEC_Q(U==1));
deriv_deltat_yp(U==1) = FDOM(U==1).^2./vs^2.*2.*(YVEC_P(U==1)-YVEC_Q(U==1));


% f = -sum(sum(DELTA_t_sq));
% grad_xp = -sum(deriv_deltat_xp - deriv_deltat_xp',2);  % note minus in the middle because we swap the order of p and q
% grad_yp = -sum(deriv_deltat_xp - deriv_deltat_xp',2);
% % 
% grad = zeros(size(x));
...
deriv_MU1_deltat = make_errorbar_mean_derivative_squared(xout_mean_twp75, DELTA_t_sq);
...
SIGMA1 = U.*SIGMA1;
deriv_SIGMA1_deltat = make_errorbar_mean_derivative_squared(xout_sigma_twp75, DELTA_t_sq);
deriv_SIGMA1_xp = deriv_SIGMA1_deltat.*deriv_deltat_xp;
deriv_SIGMA1_yp = deriv_SIGMA1_deltat.*deriv_deltat_yp;


deriv_MU1_xp = U.*deriv_MU1_xp;
deriv_MU1_yp = U.*deriv_MU1_yp;
% f = -sum(sum(MU1));
% grad_xp = -sum(deriv_MU1_xp - deriv_MU1_xp',2);  % note minus in the middle because we swap the order of p and q
% grad_yp = -sum(deriv_MU1_yp - deriv_MU1_yp',2);
% % 
% grad = zeros(size(x));
% grad(1)  =  grad_xp(2);  % i.e. becuase x1 = y1 = 0 (first event fixed at origin) and y2
% count = 2;
