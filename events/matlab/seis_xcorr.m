...
% correlation between two waveforms and uses it to estimate the sepaation
...
[noise_energy1,noise_energy2] = LOC_compute_noise(wave1,wave2,time1,time2,indwinlen,plot_par);
        ...
        switch windowtype
            case {'i','independent'}
                ...
                for i=1:numwindows
                    ...
                    sep(i,1) = maxcorr(i,1);
                    [maxcorr(i,2), tmp_xcorr(i,:)] = LOC_compute_correlation(wave1,wave2,indstart(i),indend(i),time1,time2,plot_par,noise_energy1,noise_energy2);
                end
...
function [maxcorr, tmp_xcorr] = LOC_compute_correlation(wave1,wave2,indstart,indend,time1,time2,plot_par,noise_energy1,noise_energy2)
%count = 0;
num_ts = indend-indstart;
count = -floor(num_ts/2);

% C(1) = trapz(time1(indstart:indend),wave1(indstart:indend).*wave2(indstart:indend));
% [energy1(1)] = compute_energy(wave1(indstart:indend),time1(indstart:indend));
% [energy2(1)] = compute_energy(wave2(indstart:indend),time2(indstart:indend));
...
omega2_numerator = compute_energy(grad_wave_u(indstart:indend),time_u(indstart:indend));
omega2_denominator= compute_energy(wave_u(indstart:indend),time_u(indstart:indend));
...
function     [noise_energy1,noise_energy2] = LOC_compute_noise(wave1,wave2,time1,time2,indwinlen,plot_par)
