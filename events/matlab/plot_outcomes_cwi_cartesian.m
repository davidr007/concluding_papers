...
% Now lets do some translations to put them back into the original local
% position defined by hypoDD
ind = find(hypodd(:,1) == hypodd_68(1,1));
translationx = xend_tmp(1) - hypodd(ind,5);
xend = xend_tmp(:) - repmat(translationx,nE,1);
translationy = yend_tmp(1) - hypodd(ind,6);
yend = yend_tmp(:) - repmat(translationy,nE,1);
translationz = zend_tmp(1) - hypodd(ind,7);
zend = zend_tmp(:) - repmat(translationz,nE,1);
