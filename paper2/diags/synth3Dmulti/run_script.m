...
dirlist = {'const_100perc'};
%dirlist = { 'const_050perc'; 'const_070perc'};
%========================================
% Generate the events 
tic
rand('state',0)
E =  50 + (-50- 50).*rand(nE,3);
E(1,:) = [0 0 0];  % fix first event at the centre
E(2,1) = abs(E(2,1)); E(2,2) = 0; E(2,3) = 0;% fix the second event to be on the positive x-axis
E(3,2) = abs(E(3,2)); E(3,3) = 0; 
E(4,3) = abs(E(4,3)); %120
%E(5,3) = -120;
...
for i = 1:nE-1
    for j = i+1:nE
        delta_tnorm(count) = sqrt( (E(i,1)- E(j,1))^2 +  (E(i,2)- E(j,2))^2 + (E(i,3)- E(j,3))^2)*fdom/vs;
        [ynew_mu(count), ynew_sigma(count)] = make_synthetic_muN(delta_tnorm(count));
        CWI_stat_master(count,:) = [i j 0.1 0.02 0.1 ynew_mu(count) ynew_sigma(count) fdom vs];
        count = count+1;
        % col3 = mean
        % col4 = std
        % col5 = median
...
clear CWI_stat
%========================================
% save the ttconst matrix 
% Note the we enter null values so it won't actually be used
ttconst = -99999*ones(nE,7);
ttconst(:,1) = 1:50;
...
for i = 1:ndir
    ...
    run3Dsynth_CWIopt_test(nE,opttype,npar,nrand,E)
