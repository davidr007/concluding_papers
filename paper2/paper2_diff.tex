...
}\DIFdelend \DIFaddbegin \DIFadd{arrival
}\DIFaddend time data. Synthetic examples in 2D and 3D and real earthquakes on
...
}\end{enumerate}\DIFadd{. }\DIFaddend Such errors are too large for many
...
\DIFdelend \DIFaddbegin \DIFadd{arrival-time differences computed by time-lag cross correlation of
early-onset body waves 
}\citep{dr_Ito85a, dr_Got94a, dr_Slunga95a, dr_Nadeau97a, dr_Waldhauser99a}\DIFaddend . By doing
so, \DIFdelbegin \DIFdel{they }\DIFdelend remove errors associated with velocity variations outside
...
data utilised may be the direct arrival times; \DIFdelbegin \DIFdel{travel time
...
\DIFadd{
\sigma_\tau^2 = \frac{ \left(\frac{6}{\alpha^8}+\frac{7}{\beta^8} \right) \delta_{\| fault}^2 +
2 \left(\frac{1}{\alpha^8}+\frac{2}{\beta^8} \right) \delta_{\perp fault}^2 }
{7 \left( \frac{2}{\alpha^8}+\frac{3}{\beta^8} \right)},}\end{equation} \end{linenomath*}
...
\DIFdelbegin \DIFdel{travel time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend differences. In the following derivation and in 
...
For the purpose of this work we ignore this week dependance and assume independence. Therefore, we have
...
between earthquakes. The inclusion of \DIFdelbegin \DIFdel{travel times in Combining Travel Time }\DIFdelend \DIFaddbegin \DIFadd{arrival-times in Combining Arrival-Time }\DIFaddend and CWI Constraints
...
coordinate error is 2.0\,m  \DIFaddbegin \DIFadd{(average location error of $\approx$4}\,\DIFadd{m) 
...
error of 2.8\,m \DIFaddbegin \DIFadd{(average location error of $\approx$9}\,\DIFadd{m)}\DIFaddend .
...
This is experiment is designed to mimic a realistic recording situation 
...
Figures \ref{fig:optimisationresults-2Dsynth}(a) and (b) \DIFdelbegin \DIFdel{illustrates
}\DIFdelend \DIFaddbegin \DIFadd{illustrate
}\DIFaddend the maximum $\Delta_{max}$ (top) and mean $\Delta_\mu$ (middle) of
...
solution space when all 25 optimizations are considered (\DIFdelbegin \DIFdel{gray}\DIFdelend \DIFaddbegin \DIFadd{this}\DIFaddend ). In
...
black }\DIFdelend \DIFaddbegin \DIFadd{this and
thick }\DIFaddend lines below 60\% and 30\% of the constraints, respectively.
...
illustrated by }\citet{dr_Robinson11a}\DIFadd{.
}\DIFaddend 
...
in-fault separation estimates given by \ref{eq:LitReview-fulDC-sep}.
...
of equation \ref{eq:LitReview-fulDC-sep} so that we can focus on developing 
a working algorithm and demonstrate capability 
without dealing with the complexity of 
in-fault ($\delta_{\| fault}$) and and out-of-fault 
...
Such an algorithm would be useful for cases where the earthquakes are 
believed to be in the same plane. 
}
...
experience \DIFdelbegin \DIFdel{suggest that the coda are }\DIFdelend \DIFaddbegin \DIFadd{suggests that the CWI locationsare }\DIFaddend less supportive of the presence of
...
can be conducted for and historical aftershock sequence or earthquake swarm
...
on newly installed sensors regardless of whether the event occure before, 
...
the School fo GeoSciences at the University of Edinburgh and an anonymous 
reviewer. The authors have also benefited fro }\DIFaddend comments and advice received from
...
events. Relative locations are shown for the combined CWI and \DIFdelbeginFL \DIFdelFL{travel
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{arrival
}\DIFaddendFL time inversion (left) and the inversion with \DIFdelbeginFL \DIFdelFL{travel }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{arrival }\DIFaddendFL times only
