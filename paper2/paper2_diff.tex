...
\section*{Abstract}
\end{centering}
\DIFdelbegin \DIFdel{Coda waves }\DIFdelend \DIFaddbegin \DIFadd{Coda-waves }\DIFaddend arise from scattering to form the later arriving
components of a seismogram. \DIFdelbegin \DIFdel{Coda wave }\DIFdelend \DIFaddbegin \DIFadd{Coda-wave }\DIFaddend interferometry is an emerging
tool for constraining earthquake source properties from the
intereference pattern of \DIFdelbegin \DIFdel{coda waves }\DIFdelend \DIFaddbegin \DIFadd{coda-waves }\DIFaddend between nearby events. A new
earthquake location algorithm is derived which relies on \DIFdelbegin \DIFdel{coda wave
}\DIFdelend \DIFaddbegin \DIFadd{coda-wave
}\DIFaddend based probabilistic estimates of earthquake separation. The
algorithm can be used with \DIFdelbegin \DIFdel{coda waves }\DIFdelend \DIFaddbegin \DIFadd{cod-waves }\DIFaddend alone or in tandem with \DIFdelbegin \DIFdel{travel
time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time 
}\DIFaddend data. Synthetic examples in 2D and 3D and real earthquakes on
the Calaveras Fault, California are used to demonstrate the
potential of \DIFdelbegin \DIFdel{coda waves }\DIFdelend \DIFaddbegin \DIFadd{coda-waves }\DIFaddend for locating poorly recorded earthquakes. It
is demonstrated that \DIFdelbegin \DIFdel{coda wave }\DIFdelend \DIFaddbegin \DIFadd{coda-wave }\DIFaddend interferometry: (a) outperforms
...
from microseismicity requires greater \DIFdelbegin \DIFdel{detail }\DIFdelend \DIFaddbegin \DIFadd{location accuracy }\DIFaddend than determining
...
of stations and their geometry, signal-to-noise \DIFaddbegin \DIFadd{ratio}\DIFaddend , clarity of onsets
and accuracy of the velocity model used in computing \DIFdelbegin \DIFdel{travel times}\DIFdelend \DIFaddbegin \DIFadd{arrival-times}\DIFaddend .
Uncertainties in absolute location are typically of the order of
several kilometers\DIFaddbegin \DIFadd{, primarily }\DIFaddend because they are susceptible to uncertainty in
...
\DIFdelbegin \DIFdel{uncertainty }\DIFdelend \DIFaddbegin \DIFadd{uncertainties }\DIFaddend in the ISC (International Sesimological
...
that \DIFdelbegin \DIFdel{at the local scale, }\DIFdelend absolute locations are accurate to within
5\,km with a 95\% confidence level when local networks meet \DIFdelbegin \DIFdel{a number
of station related criteria. 
}\DIFdelend \DIFaddbegin \DIFadd{the following 
criteria:
}\begin{enumerate}
\item \DIFadd{there are 10 or more stations, all within 250 km, 
}\item \DIFadd{an azimuthal gap of less than 110$^o$, 
}\item \DIFadd{a secondary azimuthal gap of less than 160$^o$, and 
}\item \DIFadd{at least one station within 30 km. 
}\end{enumerate}
\DIFaddend Such errors are too large for many
...
structure, relative location techniques \DIFaddbegin \DIFadd{typically }\DIFaddend compute locations directly
from \DIFdelbegin \DIFdel{travel time differences between two waveforms
}%DIFDELCMD < \citep{dr_Ito85a, dr_Got94a, dr_Nadeau97a, dr_Waldhauser99a}%%%
\DIFdel{. By doing
so , they remove }\DIFdelend \DIFaddbegin \DIFadd{arrival-time differences computed by time-lag cross correlation of
early-onset body waves 
}\citep{dr_Ito85a, dr_Got94a, dr_Slunga95a, dr_Nadeau97a, dr_Waldhauser99a}\DIFadd{. Doing
so removes }\DIFaddend errors associated with velocity variations outside
the local region, because such variations influence all waveforms in
\DIFdelbegin \DIFdel{the same }\DIFdelend \DIFaddbegin \DIFadd{a similar }\DIFaddend manner \citep{dr_Shearer99a}.
...
data utilised may be the direct \DIFdelbegin \DIFdel{arrival times; travel time
}\DIFdelend \DIFaddbegin \DIFadd{arrival-times; arrival-time
}\DIFaddend difference computed between picked arrivals of two waveforms; or
\DIFdelbegin \DIFdel{time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend differences inferred from time-lagged cross correlation of
...
develop an approach for locating earthquakes using \DIFdelbegin \DIFdel{coda waves}\DIFdelend \DIFaddbegin \DIFadd{coda-waves}\DIFaddend .
...
interference pattern between the \DIFdelbegin \DIFdel{coda waves. Unlike travel time
}\DIFdelend \DIFaddbegin \DIFadd{coda-waves. Unlike arrival-time
}\DIFaddend based location techniques, CWI
...
Our technique can be used on \DIFdelbegin \DIFdel{coda waves }\DIFdelend \DIFaddbegin \DIFadd{coda-waves }\DIFaddend alone or in combination with
\DIFdelbegin \DIFdel{travel times}\DIFdelend \DIFaddbegin \DIFadd{arrival-times}\DIFaddend . We begin by introducing the theory of CWI based earthquake location. 
...
and CWI in combination with \DIFdelbegin \DIFdel{travel time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend constraints.
...
where $\sigma_\tau$ is the standard deviation of the \DIFdelbegin \DIFdel{travel time
}\DIFdelend \DIFaddbegin \DIFadd{arrival-time
}\DIFaddend perturbation between the \DIFdelbegin \DIFdel{coda waves }\DIFdelend \DIFaddbegin \DIFadd{coda-waves }\DIFaddend of two earthquakes, and $\alpha$
...
\end{equation} \end{linenomath*}
\citep{dr_Snieder05a}. \DIFaddbegin \citet{dr_Snieder05a} \DIFadd{also show that for two 
...
\end{equation} \end{linenomath*}
\DIFadd{where $\delta_{\| fault}^2$ and $\delta_{\perp fault}^2$ are the separation parallel 
and perpendicular to the fault, respectively. }\DIFaddend In this paper we use 
equation \DIFaddbegin \DIFadd{\ref{eq:LitReview-g-2d-ps} for the 2D examples.
For the 3D examples we use equation
}\DIFaddend \ref{eq:LitReview-g-3d-dc} which assumes that the source mechanism
...
\end{equation} \end{linenomath*}
\DIFaddbegin \DIFadd{where $\overline{\omega^2}$ is the square of the dominant angular frequency
}\begin{equation}
\label{eq:square-of-domangfreq}
\overline{\omega^2} \DIFadd{=
\frac{\int_{t-t_w}^{t+t_w}\dot{u}_i^2(t')dt'}{\int_{t-t_w}^{t+t_w}u_i^2(t')dt'},
}\end{equation}
\DIFadd{and $\dot{u}_i$ represents the time derivative of $u_i$.
}\DIFaddend In this paper we use the autocorrelation approach of
...
450\,m separation for 1 to 5\,Hz filtered \DIFdelbegin \DIFdel{coda waves}\DIFdelend \DIFaddbegin \DIFadd{coda-waves}\DIFaddend ).
...
$P(\widetilde{\delta}_t)$ is the prior \DIFdelbegin \DIFdel{PDF }\DIFdelend \DIFaddbegin \DIFadd{probability density function (PDF) }\DIFaddend accounting for all
a-priori information. The \DIFaddbegin \DIFadd{use of $N$ in $\delta_{CWIN}$ depicts CWI separations that include noise. 
The nomenclature is adopted here to remain consistent with }\citet{dr_Robinson11a} \DIFadd{who study 
synthetically generated noise-free $\delta_{CWI}$ and relate them to noisy estimates $\delta_{CWIN}$.
The }\DIFaddend tilde above the separation parameters in
...
\citet{dr_Robinson11a} and summarised in Appendix \DIFaddbegin \DIFadd{(The Likelihood)
}\DIFaddend \ref{sec-Appendix-noisylikelihood}. With these two pieces in place
...
We seek a \DIFdelbegin \DIFdel{probability density function (PDF ) }\DIFdelend \DIFaddbegin \DIFadd{PDF }\DIFaddend which links
...
More importantly\DIFaddbegin \DIFadd{, }\DIFaddend however, the PDF shall
...
combination of \DIFdelbegin \DIFdel{travel times}\DIFdelend \DIFaddbegin \DIFadd{arrival-times}\DIFaddend , geodetic information  or CWI
separations. For example, if \DIFdelbegin \DIFdel{coda waves }\DIFdelend \DIFaddbegin \DIFadd{coda-waves }\DIFaddend are used we have
...
CWI and \DIFdelbegin \DIFdel{travel time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend data we may write $P(\mathbf{e}_1, \mathbf{e}_2,
...
\mathbf{e}_2, \mathbf{e}_3)$ where $\mathbf{\Delta}_{TT}$ \DIFdelbegin \DIFdel{represent
travel time }\DIFdelend \DIFaddbegin \DIFadd{represents
the arrival-time }\DIFaddend differences. In the following derivation and in 
\DIFaddbegin \DIFadd{the }\DIFaddend Synthetic Experiements and Relocating Earthquakes on the Calaveras Fault \DIFaddbegin \DIFadd{section }\DIFaddend we focus
on the constraints imposed by \DIFdelbegin \DIFdel{coda waves}\DIFdelend \DIFaddbegin \DIFadd{coda-waves}\DIFaddend , whereas in 
Combining \DIFdelbegin \DIFdel{Travel Time }\DIFdelend \DIFaddbegin \DIFadd{Arrival-Time }\DIFaddend and CWI Constraints we demonstrate how CWI and \DIFdelbegin \DIFdel{travel
time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend data can be combined.
...
knowledge of location translates to separation. Note\DIFaddbegin \DIFadd{, however, }\DIFaddend that the reverse is
...
location. \DIFdelbegin \DIFdel{Furthermore, since the pairwise functions are independent the joint
likelihood becomes }\DIFdelend \DIFaddbegin \DIFadd{In other words, knowing $|e1-e2|$ and $|e2-e3|$ does not mean that $|e1-e3|$ is 
...
}\DIFaddend \begin{linenomath*} \begin{equation}
\begin{array}{l}
\label{eq:-joint-likelihood4three}
P(\widetilde{\mathbf{\delta}}_{CWIN} | \mathbf{e}_1, \mathbf{e}_2, \mathbf{e}_3) \DIFdelbegin \DIFdel{=
}\DIFdelend \DIFaddbegin \DIFadd{\approx
}\DIFaddend P(\widetilde{\delta}_{CWIN,12} | \mathbf{e}_1, \mathbf{e}_2) \\
...
system when using \DIFdelbegin \DIFdel{coda waves }\DIFdelend \DIFaddbegin \DIFadd{coda-waves }\DIFaddend alone because the CWI technique constrains only event separation
between earthquakes. The inclusion of \DIFdelbegin \DIFdel{travel times in Combining Travel Time }\DIFdelend \DIFaddbegin \DIFadd{arrival-times in the Combining Arrival-Time 
}\DIFaddend and CWI Constraints \DIFaddbegin \DIFadd{section 
}\DIFaddend allows us to move to a global reference system.
...
observed \DIFdelbegin \DIFdel{coda waves}\DIFdelend \DIFaddbegin \DIFadd{coda-waves}\DIFaddend . The most likely set of locations is given by
the minimum of $L$.   In this paper we use the Polak-Ribiere
technique \citep{dr_Press87a}, a conjugate gradient method, to
minimize $L$. It uses the derivatives of $L$, derived in Appendix \DIFaddbegin \DIFadd{(Derivatives)
}\DIFaddend \ref{sec-Appendix-derivatives_ofL}, to guide the
...
Fault and in Combining \DIFdelbegin \DIFdel{Travel Time }\DIFdelend \DIFaddbegin \DIFadd{Arrival-Time }\DIFaddend and CWI Constraints.

\vspace{1em}
\subsection*{Examples 1 and 2 - 2D synthetic experiments}

We design a 2D synthetic acoustic experiment (example 1) \DIFaddbegin \DIFadd{to test the 
performance of our CWI based relative location algorithm }\DIFaddend by randomly
...
Figure \ref{fig-2D50eq-relocation_eg1}.
 We assume a local velocity of \DIFdelbegin \DIFdel{$\alpha=3,300\,$}\DIFdelend \DIFaddbegin \DIFadd{$\alpha=3300\,$}\DIFaddend ms$^{-1}$ between all
event pairs and a dominant frequency of 2.5$\,$Hz to represent
waveform data filtered between 1 and 5$\,$Hz. The \DIFdelbegin \DIFdel{CWI }\DIFdelend \DIFaddbegin \DIFadd{purpose of these
...
the CWI separation estimates directly. }\citet{dr_Robinson11a} \DIFadd{showed that
the CWI }\DIFaddend data are
defined by the dominant wavelength normalized positive bounded
Gaussian PDF with statistics $\bar{\mu}_N$ and $\bar{\sigma}_N$\DIFdelbegin %DIFDELCMD < \citep{dr_Robinson11a}%%%
\DIFdelend . 
...
coordinate error is 2.0\,m  \DIFaddbegin \DIFadd{(average location error $\approx$4}\,\DIFadd{m) 
}\DIFaddend which is small compared to the noise of
...
error of 2.8\,m \DIFaddbegin \DIFadd{(average location error $\approx$9}\,\DIFadd{m)}\DIFaddend .
...
\DIFdelbegin \DIFdel{no differences in the solution for either example 
}\DIFdelend \DIFaddbegin \DIFadd{negligble difference in the solutions indicating that neither example 
is susceptible to local minima}\DIFaddend .
...
coverage in triangulation \DIFdelbegin \DIFdel{problems }\DIFdelend such as individual earthquake
location from limited \DIFdelbegin \DIFdel{travel time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend data, or GPS positioning with few
...
rotational freedom of the cluster is in \DIFdelbegin \DIFdel{principal }\DIFdelend \DIFaddbegin \DIFadd{principle }\DIFaddend removed by the
...
The same \DIFdelbegin \DIFdel{phenomena }\DIFdelend \DIFaddbegin \DIFadd{phenomenon }\DIFaddend is observed in linear inversion where noise
...
eigenvectors with the smallest singular values \citep{dr_Aster05a}.
 Fortunately\DIFaddbegin \DIFadd{, }\DIFaddend however, combining \DIFdelbegin \DIFdel{coda waves }\DIFdelend \DIFaddbegin \DIFadd{coda-waves }\DIFaddend with
measurements of \DIFdelbegin \DIFdel{travel times }\DIFdelend \DIFaddbegin \DIFadd{arrival-times }\DIFaddend alleviates this problem and \DIFdelbegin \DIFdel{facilitate
}\DIFdelend \DIFaddbegin \DIFadd{facilitates
}\DIFaddend the removal of a local coordinate system altogether (see Combining \DIFdelbegin \DIFdel{Travel Time }\DIFdelend \DIFaddbegin \DIFadd{Arrival-Time }\DIFaddend and 
CWI Constraints). On balance\DIFaddbegin \DIFadd{, }\DIFaddend however, we gain
...
\DIFaddbegin \DIFadd{That is, we randomly select 10}\% \DIFadd{of the event pairs and remove the 
separation estimates between those pairs to create a data set with 90}\% 
\DIFadd{linkage. Then, we randomly remove 20}\% \DIFadd{of the links and so on. 
...
}\DIFaddend As with the above examples, we undertake the optimization with 25 randomly
chosen starting locations.


\DIFdelbegin \DIFdel{Figures \ref{fig:optimisationresults-2Dsynth} (a) and (b) }\DIFdelend \DIFaddbegin \DIFadd{Figure \ref{fig:optimisationresults-2Dsynth} }\DIFaddend illustrates
...
statistics for the `best' optimization solution (\DIFdelbegin \DIFdel{black}\DIFdelend \DIFaddbegin \DIFadd{thick}\DIFaddend ) and for the
solution space when all 25 optimizations are considered (\DIFdelbegin \DIFdel{gray}\DIFdelend \DIFaddbegin \DIFadd{thin}\DIFaddend ). In
...
as an indication of \DIFdelbegin \DIFdel{the }\DIFdelend inversion stability prior to optimization. A
...
20\% or lower. 
\DIFdelbegin \DIFdel{Despite their failure to converge, the locations at
...
picking a set of \DIFdelbegin \DIFdel{actual }\DIFdelend event locations for 50 earthquakes with
$-50$\,m$\leq \hat{x},\hat{y},\hat{z} \leq 50$\,m. As in the 2D case
we assume a local velocity of \DIFdelbegin \DIFdel{$v=3,300\,$}\DIFdelend \DIFaddbegin \DIFadd{$v=3300\,$}\DIFaddend ms$^{-1}$ between all event
...
\DIFdelbegin \DIFdel{$\bar{\sigma}_N = \epsilon$ and }\DIFdelend \DIFaddbegin \DIFadd{$\bar{\sigma}_N = \epsilon(\delta_t)$ (where $\epsilon(\delta_t)$ is the same 
as that used in Examples 2 and 3) and }\DIFaddend perform the optimization
...
results (\DIFdelbegin \DIFdel{gray}\DIFdelend \DIFaddbegin \DIFadd{thin}\DIFaddend ) are consistent with the best solution obtained from
all 25 starting locations (\DIFdelbegin \DIFdel{black}\DIFdelend \DIFaddbegin \DIFadd{thick}\DIFaddend ). The
best solution constrains the event locations down to 30\% of the
direct links. \DIFdelbegin \DIFdel{There is one notable difference between the 3D and 2D results. In 2D
...
resemble the actual solution. }\DIFdelend This is depicted in Figure
\ref{fig:optimisationresults-3Dsynth} by the absence of the \DIFdelbegin \DIFdel{gray and
black }\DIFdelend \DIFaddbegin \DIFadd{thin and
thick }\DIFaddend lines below 60\% and 30\% \DIFdelbegin \DIFdel{of the constraints}\DIFdelend \DIFaddbegin \DIFadd{linkage}\DIFaddend , respectively.
The \DIFdelbegin \DIFdel{reason for this difference may be due to the increased number of
degrees of freedom in 3D requiring a greater number of iterations to
converge. Nevertheless, the }\DIFdelend accurate convergence of the best
...
The optimization error is \DIFaddbegin \DIFadd{rotational in nature and 
}\DIFaddend influenced by the noise on CWI estimates
with greater $\bar{\sigma}_N$ leading to larger errors in the
solutions. \DIFdelbegin \DIFdel{When }\DIFdelend \DIFaddbegin \DIFadd{In 3D, when }\DIFaddend 70\% or more of the direct branches are used the
...
themselves to accurate \DIFdelbegin \DIFdel{travel time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend location. This makes them ideal
...
into four examples as summarised in Table \ref{tab:examples}. \DIFaddbegin \DIFadd{Waveforms, 
cross correlations and separation estimates for example Calaveras event pairs are 
illustrated by }\citet{dr_Robinson11a} \DIFadd{an are not repeated her for the sake of brevity.
}\DIFaddend 
...
is located individually using all available \DIFdelbegin \DIFdel{travel time arrivals }\DIFdelend \DIFaddbegin \DIFadd{arrival-time data }\DIFaddend and
...
windows between \DIFdelbegin \DIFdel{$2.5 \leq t \leq 20$}\DIFdelend \DIFaddbegin \DIFadd{$2.5 < t \leq 20$}\DIFaddend \,s and used to create
probabilistic constraints on event separation. We utilize the
local coordinate system introduced in \DIFdelbegin \DIFdel{Theory }\DIFdelend \DIFaddbegin \DIFadd{the Theory Section }\DIFaddend and find the
optimum relative locations using Polak-Ribiere optimization.

\DIFaddbegin \DIFadd{In this, and the following Calaveras examples, we allow the earthquakes to 
...
\DIFaddend CWI locations for the 68 events are illustrated in column two of
...
times and cross correlation computed \DIFdelbegin \DIFdel{travel time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend differences.

Absolute locations cannot be found by CWI alone. This is because of
the \DIFdelbegin \DIFdel{non-uniquesness }\DIFdelend \DIFaddbegin \DIFadd{non-uniqueness }\DIFaddend associated with translation, rotation and
...
the hypoDD location for \DIFdelbegin \DIFdel{the same }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend event. This arbitrary translation
...
combined \DIFdelbegin \DIFdel{travel time and coda wave }\DIFdelend \DIFaddbegin \DIFadd{arrival-time and coda-wave }\DIFaddend inversion.
...
experience \DIFdelbegin \DIFdel{suggest that the coda }\DIFdelend \DIFaddbegin \DIFadd{suggests that the CWI locations }\DIFaddend are less supportive of the presence of
...
(Example 6) and exploring how CWI and \DIFdelbegin \DIFdel{travel times }\DIFdelend \DIFaddbegin \DIFadd{arrival-times }\DIFaddend can be combined
(Examples 7 and 8).
...
re-locating the 68 Calaveras events  using hypoDD and \DIFdelbegin \DIFdel{coda waves
}\DIFdelend \DIFaddbegin \DIFadd{coda-waves
}\DIFaddend with a reduced number of stations. We begin with 10 stations and
repeat the process removing one at a time until a single station
remains. The 10 stations \DIFdelbegin \DIFdel{considered are shown in Figure
\ref{fig:-eqopti-Calaveras-substations} and the }\DIFdelend \DIFaddbegin \DIFadd{and there }\DIFaddend order of removal \DIFdelbegin \DIFdel{explained in Table \ref{tab:Calaveras-stationremoval}}\DIFdelend \DIFaddbegin \DIFadd{are shown in Figure
\ref{fig:-eqopti-Calaveras-substations}}\DIFaddend .
...
considered. That is, the \DIFdelbegin \DIFdel{coda wave }\DIFdelend \DIFaddbegin \DIFadd{coda-wave }\DIFaddend approach is self-consistent
regardless of the number of stations available, reinforcing our
\DIFdelbegin \DIFdel{hypothesis that coda waves }\DIFdelend \DIFaddbegin \DIFadd{claim that coda-waves }\DIFaddend can constrain location in what would
normally be regarded as a poor station network.


Figure \ref{fig-HYPODDreducesstats} illustrates the hypoDD inversion
results for seven, five and four stations. The \DIFdelbegin \DIFdel{travel time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend problem
...
locations are \DIFdelbegin \DIFdel{not }\DIFdelend \DIFaddbegin \DIFadd{less }\DIFaddend self-consistent as the number of stations is
...
As the number of stations \DIFdelbegin \DIFdel{are reduced both the CWI and }\DIFdelend \DIFaddbegin \DIFadd{is reduced neither the CWI nor }\DIFaddend hypoDD
techniques are \DIFdelbegin \DIFdel{not }\DIFdelend able to re-locate all events. To use the coda
...
there must be at least one station that records it\DIFaddbegin \DIFadd{, }\DIFaddend and at least one
other earthquake\DIFaddbegin \DIFadd{, }\DIFaddend sufficiently well to apply CWI. Fortunately, we can
...
\DIFdelbegin \DIFdel{coda waves }\DIFdelend \DIFaddbegin \DIFadd{coda-waves }\DIFaddend constrain a decreasing number of events until at one
...
data are analyzed to ensure that there \DIFdelbegin \DIFdel{exists travel time
}\DIFdelend \DIFaddbegin \DIFadd{exist arrival-time
}\DIFaddend differences for each event and at least one other earthquake. This
is analogous to the situation for the \DIFdelbegin \DIFdel{coda wave }\DIFdelend \DIFaddbegin \DIFadd{coda-wave }\DIFaddend technique. The
hypoDD program also has a secondary identification phase in which
events that can not be located sufficiently \DIFaddbegin \DIFadd{well }\DIFaddend are rejected during the
...
illustrate a statistical comparison of the CWI and hypoDD \DIFdelbegin \DIFdel{reduced
station locations 
}\DIFdelend \DIFaddbegin \DIFadd{locations 
with a reduced number of stations }\DIFaddend to those using hypoDD with all available data. For
...
\vspace{2em}
\begin{centering}
\section*{Combining \DIFdelbegin \DIFdel{Travel Time }\DIFdelend \DIFaddbegin \DIFadd{Arrival-Time }\DIFaddend and CWI Constraints}
\end{centering}
\label{sec:CalaverasLoc-CWIandTT}
In Examples 5 and 6 we compare the location of the Calaveras
earthquakes using \DIFdelbegin \DIFdel{coda wave }\DIFdelend \DIFaddbegin \DIFadd{coda-wave }\DIFaddend and arrival time based constraints
independently. Since the arrival time (direct or difference) and
\DIFdelbegin \DIFdel{coda wave }\DIFdelend \DIFaddbegin \DIFadd{coda-wave }\DIFaddend data
 ...
 both CWI and \DIFdelbegin \DIFdel{travel time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend data.

We do not propose a new technique for earthquake location using
\DIFdelbegin \DIFdel{travel time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend differences. Rather, we exploit the information created
...
\end{equation} \end{linenomath*}
is the most likely location as determined using the \DIFdelbegin \DIFdel{travel time
}\DIFdelend \DIFaddbegin \DIFadd{arrival-time
}\DIFaddend data, and
\begin{linenomath*} \begin{equation}
...
\end{equation} \end{linenomath*}
is the covariance matrix. \DIFdelbegin \DIFdel{In this paper we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend define the mean location
...
ln\left[P(\mathbf{e}_i|\Delta_{TT})\right]
\end{equation} \end{linenomath*}
incorporates the \DIFdelbegin \DIFdel{travel time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend constraints and
\begin{linenomath*} \begin{equation}
\label{eq-codacomponent} \sum_{i=1}^{n-1} \sum_{j=i+1}^n
ln\left[P(\delta_{CWIN}|\mathbf{e}_i,\mathbf{e}_j)\right]
\end{equation} \end{linenomath*}
 the \DIFdelbegin \DIFdel{coda waves}\DIFdelend \DIFaddbegin \DIFadd{coda-waves}\DIFaddend .
...
Combining the \DIFdelbegin \DIFdel{travel time and coda wave }\DIFdelend \DIFaddbegin \DIFadd{arrival-time and coda-wave }\DIFaddend data offers two advantages.
...
\DIFdelbegin \DIFdel{travel time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend constraints in the form of equation
...
faster and more stable convergence. Consequently, we no longer \DIFdelbegin \DIFdel{have
}\DIFdelend \DIFaddbegin \DIFadd{need
}\DIFaddend to consider multiple randomly chosen starting locations.

\vspace{1em}
\subsection*{Example 7 - Combining \DIFdelbegin \DIFdel{travel time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend and CWI constraints}
Figure
\ref{fig-68Calaverasevents_ttandcoda1} illustrates the earthquake
locations obtained when we combine the \DIFdelbegin \DIFdel{travel time and coda wave
}\DIFdelend \DIFaddbegin \DIFadd{arrival-time and coda-wave
}\DIFaddend data using all data (left) and five stations (right). The linear
...
the \DIFdelbegin \DIFdel{coda waves }\DIFdelend \DIFaddbegin \DIFadd{coda-waves }\DIFaddend introduce a scatter around these streaks. That is,
...
on linear features and the \DIFdelbegin \DIFdel{coda waves }\DIFdelend \DIFaddbegin \DIFadd{coda-waves }\DIFaddend voracity to push them away
...
\subsection*{Example 8 - Combining CWI and \DIFdelbegin \DIFdel{travel times }\DIFdelend \DIFaddbegin \DIFadd{arrival-times }\DIFaddend when the \DIFdelbegin \DIFdel{travel times }\DIFdelend \DIFaddbegin \DIFadd{arrival-times }\DIFaddend constrain a limited number of events}
...
\DIFdelbegin \DIFdel{travel time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend data when the temporary network is in-situ and using
\DIFdelbegin \DIFdel{coda waves }\DIFdelend \DIFaddbegin \DIFadd{coda-waves }\DIFaddend from network stations when the deployment is absent. The
...
$t$ days \DIFdelbegin \DIFdel{is given }\DIFdelend \DIFaddbegin \DIFadd{can be modeled }\DIFaddend by the modified Omori formula
\begin{linenomath*} \begin{equation}
\label{eq:CumOmori}
...
\end{equation} \end{linenomath*}
This \DIFdelbegin \DIFdel{idea }\DIFdelend is illustrated in Figure \ref{fig:Omorifigure} which shows
the best fitting Omori Formula separated into segments before\DIFdelbegin \DIFdel{(gray), during 
(black) and after (gray) }\DIFdelend \DIFaddbegin \DIFadd{, during 
and after }\DIFaddend the pseudo temporary
deployment.
...
consider the \DIFdelbegin \DIFdel{travel time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend constraints on half (34) of the earthquakes
and incorporate \DIFdelbegin \DIFdel{coda wave }\DIFdelend \DIFaddbegin \DIFadd{coda-wave }\DIFaddend data from a single station for all 68
...
is similar to the combined inversion when all \DIFdelbegin \DIFdel{travel time data is
}\DIFdelend \DIFaddbegin \DIFadd{arrival-time data are
}\DIFaddend incorporated (see Fig. \ref{fig-68Calaverasevents_ttandcoda1}). The
slight increase in scatter observed here can be explained by the
events with no \DIFdelbegin \DIFdel{travel time }\DIFdelend \DIFaddbegin \DIFadd{arrival-time }\DIFaddend constraints and the tendency of the coda
to push events away from streaks.

Remarkably, the combined \DIFdelbegin \DIFdel{coda wave and travel time }\DIFdelend \DIFaddbegin \DIFadd{coda-wave and arrival-time }\DIFaddend inversion locates
all 68 earthquakes to an accuracy similar to the inversions with all
data. In contrast when \DIFdelbegin \DIFdel{travel time data is }\DIFdelend \DIFaddbegin \DIFadd{arrival-time data are }\DIFaddend used alone it is only
possible to locate the 34 events recorded by the pseudo temporary
deployment. This
ability of  \DIFdelbegin \DIFdel{coda waves }\DIFdelend \DIFaddbegin \DIFadd{coda-waves }\DIFaddend to constrain the location of events recorded
...
\DIFdelbegin \DIFdel{Coda wave }\DIFdelend \DIFaddbegin \DIFadd{Coda-wave }\DIFaddend interferometry is an emerging technique for constraining
earthquake location. The technique relies on the interference
between \DIFdelbegin \DIFdel{coda waves }\DIFdelend \DIFaddbegin \DIFadd{coda-waves }\DIFaddend of closely located events and is hence useful for
studying earthquake clusters and/or aftershock sequences. \DIFdelbegin \DIFdel{Coda wave
}\DIFdelend \DIFaddbegin \DIFadd{Coda-wave
}\DIFaddend constraints are independent of \DIFdelbegin \DIFdel{travel times }\DIFdelend \DIFaddbegin \DIFadd{arrival-times }\DIFaddend and can be used in
...
\DIFdelbegin \DIFdel{Coda wave }\DIFdelend \DIFaddbegin \DIFadd{Coda-wave }\DIFaddend interferometry offers a new technique for understanding
earthquakes in intraplate areas with sparse networks and poor
azimuthal coverage. In particular, the ability to combine \DIFdelbegin \DIFdel{coda wave
constraints with travel times }\DIFdelend \DIFaddbegin \DIFadd{coda-wave
constraints with arrival-times }\DIFaddend makes it possible to link well
...
intraplate seismicity and earthquake hazard. \DIFaddbegin \DIFadd{Importantly, this analysis 
...
}\citet{dr_Curtis12a_EPSL} \DIFadd{that utilizes interferometry to obtain seismic signals
...
at Geoscience Australia\DIFdelbegin \DIFdel{; as well as }\DIFdelend \DIFaddbegin \DIFadd{, the BSSA editorial team, Andrew Curtis from 
the School of GeoSciences at the University of Edinburgh and an anonymous 
reviewer. The authors have also benefited from }\DIFaddend comments and advice received from
...
\DIFaddbegin \bibitem[{\textit{Curtis et~al.}(2012)}]{dr_Curtis12a_EPSL}
\DIFadd{Curtis, A., Behr, Y., Entwistle, E., Galetti, E., Townend J. Bannister, S. (2012),
The benefit of hindsight in observational science: }{\DIFadd{R}}\DIFadd{etrospective
seismological observations, }\epsl\DIFadd{, \textit{345--348}, 212--220.
}

\DIFaddend \bibitem[{\textit{Deichmann and Garcia-{F}ernandez}(1992)}]{dr_Deichmann92a}
...
\DIFaddbegin \bibitem[{\textit{Slunga et~al.}(1995)}]{dr_Slunga95a}
\DIFadd{Slunga, R., R}{\"o}\DIFadd{gnvaldsson, }{\DIFadd{S. Th.}} \DIFadd{and B}{\"o}\DIFadd{dvarsson, R. (1995),
Absolute and relative locations of similar events with application to microearthquakes in southern }{\DIFadd{I}}\DIFadd{celand,
}\gji\DIFadd{,\textit{123}(2),409--419.
}

\DIFaddend \bibitem[{\textit{Snieder}(1999)}]{dr_Snieder99a}
...
station coverage.}} %DIFAUXCMD
%DIFDELCMD < \label{tab:Calaveras-stationremoval}
%DIFDELCMD < \begin{tabular}{|r|l|}
%DIFDELCMD < \hline
%DIFDELCMD < %%%
\DIFdel{Number of }%DIFDELCMD < & %%%
\DIFdel{Station Names}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{Stations  }%DIFDELCMD < & \\
%DIFDELCMD < \hline
%DIFDELCMD < %%%
\DIFdel{10 }%DIFDELCMD < & %%%
\DIFdel{CCO, JCB, JST, CMH, HSP, JAL, CSC, JST, CAD, JHL, JRR}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{9  }%DIFDELCMD < & %%%
\DIFdel{CCO, JCB, JST, CMH, HSP, JAL, CSC, JST, CAD, JHL}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{8  }%DIFDELCMD < & %%%
\DIFdel{CCO, JCB, JST, CMH, HSP, JAL, CSC, JST, CAD}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{7  }%DIFDELCMD < & %%%
\DIFdel{CCO, JCB, JST, CMH, HSP, JAL, CSC }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{6  }%DIFDELCMD < & %%%
\DIFdel{CCO, JCB, JST, CMH, HSP, JAL }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{5  }%DIFDELCMD < & %%%
\DIFdel{CCO, JCB, JST, CMH, HSP }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{4  }%DIFDELCMD < & %%%
\DIFdel{CCO, JCB, JST, CMH }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{3  }%DIFDELCMD < & %%%
\DIFdel{CCO, JCB, JST }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{2  }%DIFDELCMD < & %%%
\DIFdel{CCO, JCB }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{1  }%DIFDELCMD < & %%%
\DIFdel{CCO }%DIFDELCMD < \\
%DIFDELCMD < \hline
...
earthquakes are shown for the purpose of orientation only. \DIFaddbeginFL \DIFaddFL{In this and subsequent
...
the order \DIFdelbeginFL \DIFdelFL{in Table \ref{tab:Calaveras-stationremoval} and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{defined by }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{events
relocated}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{bracketed numbers}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{That is, JRR is the first station to 
be removed, JHL is the second and so on. }\DIFaddendFL Events are indicated with circles.}
...
\DIFaddbeginFL \DIFaddFL{Axes as defined in Figure \ref{fig-69Calaverasevents_eg1}.
}\DIFaddendFL \hspace{20em}}
...
stations. \DIFaddbeginFL \DIFaddFL{Axes as defined in Figure \ref{fig-69Calaverasevents_eg1}. 
}\DIFaddendFL \hspace{20em} }
...
and hypoDD inversions as a function of the \DIFaddbeginFL \DIFaddFL{number of }\DIFaddendFL stations considered
...
(right). \DIFaddbeginFL \DIFaddFL{Axes as defined in Figure \ref{fig-69Calaverasevents_eg1}.}\DIFaddendFL } 
...
\caption{\DIFdelbeginFL \DIFdelFL{Cumulative }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Modeled cumulative }\DIFaddendFL number of aftershocks for the
...
by ignoring data from all but station CCO for 50\% (or 34) of the \DIFaddbeginFL \DIFaddFL{68
}\DIFaddendFL events. Relative locations are shown for the combined CWI and \DIFdelbeginFL \DIFdelFL{travel
time }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{arrival-time 
}\DIFaddendFL inversion (left) and the inversion with \DIFdelbeginFL \DIFdelFL{travel times }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{arrival-times }\DIFaddendFL only
...
consistent with \DIFaddbeginFL \DIFaddFL{Figure \ref{fig-69Calaverasevents_eg1}. Axes as defined 
in }\DIFaddendFL Figure \ref{fig-69Calaverasevents_eg1}.}
