...
algorithm can be used with cod-waves alone or in tandem with arrival
...
\end{enumerate}. Such errors are too large for many
...
\citep{dr_Ito85a, dr_Got94a, dr_Slunga95a, dr_Nadeau97a, dr_Waldhauser99a}. By doing
so, remove errors associated with velocity variations outside
...
arrival-time differences. In the following derivation and in 
...
between earthquakes. The inclusion of arrival-times in Combining Arrival-Time and CWI Constraints
...
coordinate error is 2.0\,m  (average location error of $\approx$4\,m) 
...
error of 2.8\,m (average location error of $\approx$9\,m).
...
Figures \ref{fig:optimisationresults-2Dsynth}(a) and (b) illustrate
...
solution space when all 25 optimizations are considered (this). In
...
\ref{fig:optimisationresults-3Dsynth} by the absence of the this and
thick lines below 60\% and 30\% of the constraints, respectively.
...
illustrated by \citet{dr_Robinson11a}.
...
experience suggests that the CWI locationsare less supportive of the presence of
...
reviewer. The authors have also benefited fro comments and advice received from
...
events. Relative locations are shown for the combined CWI and arrival
time inversion (left) and the inversion with arrival times only
